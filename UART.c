...
void UART0_Init(void)
{
	...
	UART0_IBRD_R = 104; //integer part = 43
	UART0_FBRD_R = 11; //fractional part = 0.402*64 = 26
	UART0_LCRH_R = 0x70; //8-bit data, no parity, one stop, FIFo.s enabled
	UART0_CTL_R |= 0x301; //enable the UART
	//unlocking PORTA
...
void UART2_Init(void)
{
	...
	UART2_IBRD_R = 104; //integer part = 520
	UART2_FBRD_R = 11; //fractional part = 0.8333*64 = 53
	UART2_LCRH_R = 0x70; //8-bit data, no parity, one stop, FIFo.s enabled
	UART2_CTL_R |= 0x301; //enable the UART
	//unlocking PORTD
	...
	GPIO_PORTD_PCTL_R |= 0x11000000; //configure PD6 & PD7 as Rx & Rx of UART2
